...
List bf_cpp(NumericVector y, int N, double beta, double alpha, double sigma) {
  ...
  NumericMatrix ancestorIndices(tmax+1, N);
  IntegerVector ix(N);
  ...
  ancestorIndices.row(0) = seq_len(N) - 1;  // 0 based
  weightsnorm.row(0) = seq(1 / N, N);
  // Main loop
  for (int t=1; t < (tmax+1); t++){
    // Resample: Sample indices based on weights and use them to resample the columns of particle
    ix = sample(N, N, true, (NumericVector)weightsnorm.row(t-1));
    for (int j=0; j < N; j++){
...
// [[Rcpp::export(name="pmmh_cpp_bf_sigma")]]
List pmmh(double thetastart, int niter, int N, NumericVector y, int burnin, double beta, double sigma){  // N is the number of particles
  ...
  for (int i=0; i < burnin; i++){
    ...
    // check
    if (std::abs(theta_candidate) < 1.0){
    ...
    if (logu[i] <= logm_candidate + logp(theta_candidate) - logm - logp(theta) + logqeval(theta, theta_candidate) - logqeval(theta_candidate, theta) + log((std::abs(theta_candidate) < 1.0))){
      // Accept!
