...
arma::mat particle_filter(arma::vec &y, arma::vec &theta, int &n_particles) {
  ...
  arma::mat x_hat_filtered = arma::randu<arma::vec>(T + 1);
  ...
  for (int t = 1; t <= T; t++) {
    ...
    NumericVector resampled_particles =
        Rcpp::sample(as<NumericVector>(wrap(particles.col(t - 1))), n_particles,
                     true, as<NumericVector>(wrap(weights_1_norm)));
    ...
    // RESAMPLE
    resampled_particles =
        Rcpp::sample(as<NumericVector>(wrap(particles.col(t))), n_particles,
                     true, as<NumericVector>(wrap(weights_2_norm)));
