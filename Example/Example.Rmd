...
#saveRDS(data,"simulated_data.RDS") # save dataset
```
...
# saveRDS(out, 'really_good_output.RDS')
```

Load saved data and PMH output.
```{r}
data <- readRDS("simulated_data.RDS")
y <- data[-1, 2]
out <- readRDS('really_good_output.RDS')
```

Compute the mean of the parameter estimates from iteration $200$ to $800$.
```{r}
# sigma
mean(out$samples[200:800])

# alpha
mean(out$alphasamples[200:800])
```
...
# ggsave("true_x.png",height = 5, width=8,units = "cm")
```
...
# ggsave("true_y.png",height = 5, width=8,units = "cm")
```

### Trace plots
```{r, echo=FALSE}
samples <- data.frame(iteration = 1:length(out$samples),
                      alpha=out$alphasamples,
                      sigma=out$samples)

ggplot(data = samples, aes(x = iteration, y = alpha))+
  geom_line(color = "#264653", size = 0.4)+
  geom_hline(yintercept = alpha_true,linetype = 'dashed', size = 0.4)+
  labs(y = expression(alpha))+
  scale_y_continuous(limits=c(0,1),breaks=c(seq(0,1,by=0.2),alpha_true),labels=c(seq(0,1,by=0.2),alpha_true))+
  theme(panel.grid = element_blank(),
        panel.background = element_blank(),
        text = element_text(family="LM Roman 10", size=10),
        axis.line=element_line(size = 0.3),
...
# ggsave("trace_alpha.png",height = 5, width=8,units = "cm")

ggplot(data = samples, aes(x = iteration, y = sigma))+
  geom_line(color = "#e76f51", size = 0.4)+
  geom_hline(yintercept = sigma_true, linetype = 'dashed', size = 0.4)+
...
# ggsave("trace_sigma.png",height = 5, width=8,units = "cm")
```
...
ggplot(data = samples)+
  ...
  geom_vline(xintercept = alpha_true,linetype = 'dashed',size = 0.4)+
  labs(x = expression(alpha), y="")+
  scale_x_continuous(limits=c(0,1),breaks=c(seq(0,1,by=0.2),alpha_true),labels=c(seq(0,1,by=0.2),alpha_true))+
  theme(panel.grid = element_blank(),
        panel.background = element_blank(),
        text = element_text(family="LM Roman 10", size=10),
        axis.line=element_line(size = 0.3),
...
# ggsave("posterior_alpha.png",height = 5, width=8,units = "cm")

ggplot(data = samples)+
  geom_histogram(aes(sigma), bins=25, color = "#e76f51", fill="#e76f51",size = 0.4,alpha=0.4)+
  geom_vline(xintercept = sigma_true,linetype = 'dashed',size = 0.4)+
  labs(x = expression(sigma),y="")+
  scale_x_continuous(limits=c(0.5,2.5),breaks=c(seq(0.5,2.5,by=0.25),sigma_true),labels=c(seq(0.5,2.5,by=0.25),sigma_true))+
  theme(panel.grid = element_blank(),
        panel.background = element_blank(),
        text = element_text(family="LM Roman 10", size=10),
        axis.line=element_line(size = 0.3),
...
# ggsave("posterior_sigma.png",height = 5, width=8,units = "cm")
```
...
```{r, echo=FALSE}
# x_filtered_init <- bf_cpp(y, N, beta = beta_true, alpha = 0.3, sigma = 0.5)$filtered_states
# saveRDS(x_filtered_init, "x_filtered_init.RDS")
x_filtered_init <- readRDS("x_filtered_init.RDS")

data.frame(state = 1:dim(out$final_states)[1],
...
# ggsave("states_init.png",height = 5, width=8,units = "cm")
```

State estimates based on the particle estimates at iteration $50$.
```{r, echo=FALSE}
# x_filtered_at_iter_50 <- bf_cpp(y, N, beta = beta_true, alpha = samples$alpha[50], sigma = samples$sigma[50])$filtered_states
# saveRDS(x_filtered_at_iter_50, "x_filtered_at_iter_50.RDS")

x_filtered_at_iter_50 <- readRDS("x_filtered_at_iter_50.RDS")
...
# ggsave("states_50.png",height = 5, width=8,units = "cm")
```

State estimates based on the particle estimates at iteration $100$.
```{r, echo=FALSE}
# x_filtered_at_iter_100 <- bf_cpp(y, N, beta = beta_true, alpha = samples$alpha[100], sigma = samples$sigma[100])$filtered_states
# saveRDS(x_filtered_at_iter_100, "x_filtered_at_iter_100.RDS")

x_filtered_at_iter_100 <- readRDS("x_filtered_at_iter_100.RDS")
...
# ggsave("states_100.png",height = 5, width=8,units = "cm")
```

State estimates based on the final parameter estimates.
```{r, echo=FALSE}
data.frame(state = 1:dim(out$final_states)[1],
                 mean_state_estimate=apply(out$final_states, 1, mean),
                 real_state = data[,1]) %>% 
  ggplot()+
...
# ggsave("states_final.png",height = 5, width=8,units = "cm")
```
...
#ggsave("acf_alpha.png",height = 5, width=8,units = "cm")
```
...
#ggsave("acf_sigma.png",height = 5, width=8,units = "cm")
```
