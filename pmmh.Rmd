...
```{r}
plot(1:(tmax+1), data[, 2], type='l', main='Observations', xlab='t', ylab='x')
```
...
sourceCpp("bf_cpp.cpp")
N <- 200
out <- pmmh_cpp_bf(thetastart = 0.8, 
                   niter=700, 
                   N=N, 
                   y=y,
                   burnin=100,
...
```{r}
oo = pr(N, 0.8, sigma_true)
tr(oo, 0.8, sigma_true)
```


```{r}
bf_cpp(y = y, N = N, beta = beta_true, alpha = alpha_true, sigma = 0.7)
```
