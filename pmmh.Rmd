...
```{r}
out <- APF(data[, 2], 300, c(0.8281, 0.5, 1.0))
plot(out$filtered_states, col='red', type='l')
```


```{r, fig.heigh=3, fig.heigh=3}
plot(data[, 1], type="l")
```
...
```{r}
out <- pmmh_cpp(0.5, 500, 300, data[, 2])
```
...
```{r}
out <- pmmh_cpp_bf(thetastart = 5.0, 
                   niter=700, 
                   N=200, 
                   y=data[, 2],
                   burnin=100)
