---
title: 'APPENDX: SMC Package Tutorial'
output:
...
pdf("Figures/Tutorial/particle_fit.pdf")
grid.arrange(bsf_pl, apf_pl, nrow = 1)
dev.off()
caption2 <- paste("Results of running running the BSF and APF on the synthetic", 
...
```{r eval = F}
pmmh2_fit <- SMC::pmmh2(10000, data$y, 400, c(0.5, 0.5, 0.5), 0.045)
```
...
```{r}
acceptance_rate <- pmmh2_fit$Accepted / nrow(pmmh2_fit$chain[-(1:5000),])
...
```{r include = F}
tr_plot <- function(chains, i, pname) {
  df <- data.frame("t" = 1:nrow(chains), "y" = chains[,i])
  pl <- ggplot(data = df) +
        geom_line(aes(x = t, y = y), color = "#264653", size = 0.4) +
...
alpha_pl <- tr_plot(pmmh2_fit$chain, 1, "alpha")
beta_pl <- tr_plot(pmmh2_fit$chain, 2, "beta")
sigma_pl <- tr_plot(pmmh2_fit$chain, 3, "sigma")
...
sigma_pl
dev.off()
caption3 <- "Trace plots for each model parameter; generated using pmmh2()."
```
...
The trace plots indicate that each parameters has converged to its stationary
regime. The ACF plots should also decay to $0$ (here we take a burn in of $500$
iterations).

```{r include = F}
burn_in <- 5000
chain <- pmmh2_fit$chain[-(1:burn_in),]
ggplot.corr <- function(data, pname, lag.max = 100, ci = 0.95, large.sample.size = TRUE, horizontal = TRUE,...) {
...
acf_alpha <- ggplot.corr(chain[,1], "alpha", lag.max = 200, large.sample.size = FALSE)
acf_beta <- ggplot.corr(chain[,2], "beta", lag.max = 200, large.sample.size = FALSE)
acf_sigma <- ggplot.corr(chain[,3], "sigma", lag.max = 200, large.sample.size = FALSE)
...
acf_sigma
dev.off()
caption4 <- "ACF plots for each model parameter; generated using pmmh2()."
```
...
And finally we can plot a histogram of the approximated posterior samples:
