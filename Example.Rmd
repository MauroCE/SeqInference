...
# Plot the data:
pl_data <- ggplot(cbind("t" = 1:tmax, data)) +
           geom_line(aes(x = t, y = x, col = "States")) +
           geom_point(aes(x = t, y = y, col = "Observations")) +
           theme(legend.title = element_blank(), legend.position = c(0.9, 0.9)) +
           ylab("")
# saveRDS(data,"Data/sv_example_data.RDS") # save dataset.
# png("Figures/sv_example.png")
pl_data
# dev.off()
```
...
data.frame(state = 1:dim(mh_results$final_states[-1,])[1],
...
```{r, echo=FALSE}
data.frame(state = 1:dim(mh_results$final_states[-1,])[1],
                 mean_state_estimate=rowMeans(mh_results$final_states)[-1],
                 real_state = data[,1]) %>% 
  ggplot()+
